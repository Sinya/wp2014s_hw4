				    ...
				    // var album = response.data[i];
						...
						// function getMyPhoto(album) {
				   	  	// console.log("hihi");
								...
								$("#photo").append("<img id="+photo.id + " src=" + image.src  + " crossorigin=" + "\"Anonymous\"" + " class="+ "\"img\"" +" width=" + photo.width/2 +" height=" + photo.height/2 + " onClick=" + "photoClick(" + photo.id + ")" + ">");
...
jQuery(document).ready(function() {
    ...
    jQuery( "#slider" ).slider({
        ...
        slide: function(event, ui) {
            ...
            ctx.clearRect(0,0,canvasWidth,canvasHeight); //移除canvas起始的內容
            ctx.drawImage(selectIMG,canMouseX-selectIMG.width/2,canMouseY-selectIMG.height/2,size,size / ASPECT);//從XY軸0，0值開始畫如selectIMG
			ctx.drawImage(img2,0,0); //劃入img2
