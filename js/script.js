...
// =================================================================================
	...
	ctx.fillText(inputedText, 300,300); //字體也可以依據滑鼠游標移動，所輸入的值可自行調整，若不想移動輸入的字體，可以把它改成（inputedText,0,0)X Y軸 0，0的位置
	});
...
function getMyUrl() {
	...
	if (selectIMG !== undefined) {
			...
			// ctx.drawImage(img2,0,0); //劃入img2
			s.addShape(new Shape(0,0,img.src,540,500));
...
function PostImageToFacebook(authToken) {
    ...
    fd.append("message", "Hahaha^_^");//輸入的訊息
    try {
        $.ajax({
